...
class CanvasClient:
    ...
    def fetch_courses(self, per_page: int = 100) -> List:
        ...
        """
        url = f"{self.api_url}/courses?per_page={per_page}"
        response = requests.get(
            url,
            headers=self.headers,
        )
    ...
    def fetch_external_tool_by_course(
        self, course_id: str, external_tool_id: str
    ) -> json:
    ...
    def fetch_external_tool_by_account(
        self, account_id: str, external_tool_id: str
    ) -> json:
    ...
    def update_assignment(
        self, course_id: str, assignment_id: str, assignment: dict
    ) -> json:
        ...
        """
        url = f"{self.api_url}/courses/{course_id}/assignments/{assignment_id}"
        response = requests.put(url, headers=self.headers, data=assignment)
